...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        String appContent, appContentPath, appImport;
        try(Scanner scanner=new Scanner(System.in)){
            ...
            Files.write(Paths.get("recorder.project"), projectName.getBytes(StandardCharsets.UTF_8));
            ...
            try(Connection connect=database.getConnexion(credentials)){
                ...
                for(int i=0;i<models.length;i++){
                    ...
                    if(language.getAppContent()!=null){
                        appContent+=language.getAppContent().getLink();
                        appContent=appContent.replace("[projectNameMaj]", HandyManUtils.majStart(projectName));
                        appContent=appContent.replace("[projectNameMin]", HandyManUtils.minStart(projectName));
                        appContent=appContent.replace("[classNameMin]", HandyManUtils.minStart(entities[i].getClassName()));
                        appContent=appContent.replace("[classNameMaj]", HandyManUtils.majStart(entities[i].getClassName()));
                        appContent=appContent.replace("[classNameformattedMin]", HandyManUtils.minStart(HandyManUtils.formatReadable(entities[i].getClassName())));
                        appContent=appContent.replace("[classNameformattedMaj]", HandyManUtils.majStart(HandyManUtils.formatReadable(entities[i].getClassName())));
                    }

                    if(language.getAppImport()!=null){
                        appImport+=language.getAppImport().getImports();
                        appImport=appImport.replace("[classNameMaj]", HandyManUtils.majStart(entities[i].getClassName()));
                    }
                }
                ...
                if (language.getAppContent()!=null) {
                    appContentPath=language.getAppContent().getPath().replace("[projectNameMaj]", HandyManUtils.majStart(projectName));
                    appContentPath=appContentPath.replace("[projectNameMin]", HandyManUtils.minStart(projectName));
                    System.out.println(appContentPath);
                    HandyManUtils.overwriteFileContent(appContentPath, HandyManUtils.getFileContent(appContentPath).replace("[appContent]", appContent));
                    HandyManUtils.overwriteFileContent(appContentPath, HandyManUtils.getFileContent(appContentPath).replace("[appImport]", appImport));
                }
